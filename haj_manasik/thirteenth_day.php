...
<head>
    ...
    $(document).ready(function() {
        ...
        $.get('../php/retrieveManasek.php', { dateDay: 'الثالث' }, function(data) {
...
function checkCheckboxes() {
    ...
    $.get('../php/retrieveUserManasek.php', { userEmail: userEmail }, function(data) {
        ...
        // Flag to check if the last checkbox is checked
        var isLastChecked = false;
        ...
        userManasek.forEach(function(record) {
            ...
            if (checkbox.length > 0) {
                ...
                if (nextCheckbox.length > 0) {
                    ...
                    // Set flag if last checkbox is checked
        ...
        // Redirect to the same page if the last checkbox is checked
        if (isLastChecked) {

          var parameterValue = " تقبل الله حجك وسعيك وجعل حجك مبروراً وذنبك مغفوراً"; // Replace "Your Parameter Value" with the value you want to pass
          // Redirect the user to the "end of manasek" page with the parameter
          window.location.href = 'Endmanasek.php?param=' + encodeURIComponent(parameterValue);
        }
    });
}
...
function itemChecked(checkbox) {
    if (checkbox.checked) {
        ...
        $.post('../php/insertUserManasek.php', {
            ...
            // Check if the current checkbox is the last one
            var checkboxes = $('input[type="checkbox"]');
            var currentIndex = checkboxes.index(checkbox);
            var totalCheckboxes = checkboxes.length;
            
            if (currentIndex === totalCheckboxes - 1) {
...
<body id="eighth">
    ...
    <h1 id="thirteen">اليوم الثالث عشر من ذي الحجة</h1>
    ...
    <a href="eighth_day.php" style="display: inline-block;margin-right: 70%;">
